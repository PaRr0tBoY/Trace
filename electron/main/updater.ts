...
/**
 * Detects whether the app is packaged and running as a Microsoft Store
 * (MSIX) build. Store builds must not self-update — updates flow through
 * the Store, and electron-updater is disabled there to stay compliant.
 */
...
function checkGitHubReleaseFast(): Promise<{ tag_name?: string } | null> {
  const { promise, resolve } = Promise.withResolvers<{ tag_name?: string } | null>()
  try {
    const request = net.request({
      method: 'GET',
      url: 'https://api.github.com/repos/PaRr0tBoY/Trace/releases/latest'
    })
    request.setHeader('User-Agent', 'Trace-App')
    ...
    request.on('response', (response) => {
      ...
      response.on('end', () => {
        clearTimeout(timer)
        try {
          resolve(response.statusCode === 200 ? (JSON.parse(body) as { tag_name?: string }) : null)
        } catch {
          resolve(null)
        }
      })
...
export function initAutoUpdater(): void {
  ...
  try {
    ...
    const autoUpdatesEnabled = loadSettings().autoUpdates !== false
