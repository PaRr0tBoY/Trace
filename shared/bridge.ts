 ...
 */
import type { Settings, TaskProposal, TaskDto, TaskPatch, UnlinkTarget, LocalModelSource, LocalModelStatus, SwitcherEntryDto, Note, NoteDto, NotePatch } from './types'
...
export interface EdgeApi {
  /* Renderer -> Main */
  loadState: () => Promise<{ items: import('./types').ClipboardItemDto[]; station: import('./station').StationEntryDto[]; settings: Settings; version: string; tasks: TaskDto[]; notes: NoteDto[]; isStoreBuild: boolean }>
