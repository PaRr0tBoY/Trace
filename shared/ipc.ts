 ...
 */
import type { AppRef, ClipboardItemDto, DragRequest, IgnoreReason, LocalModelSource, LocalModelStatus, MemoryConflictResolution, MemoryFactPanelPayload, MemoryUserState, MergeResult, Note, NoteDto, NotePatch, ProviderConfig, Settings, TaskProposal, TaskDto, TaskPatch, UnlinkTarget } from './types'
...
export interface InvokeMap {
  /** Returns the full current item list + settings + tasks + notes on startup. */
  'state:load': { args: []; result: { items: ClipboardItemDto[]; settings: Settings; version: string; tasks: TaskDto[]; notes: NoteDto[] } }
